...
def play_recording(request: PlayRequest):
    ...
    if recorder.last_play_end_time and (now - recorder.last_play_end_time) < PLAY_COOLDOWN:
        return {
            "status": "error",
            "message": "回放刚结束，请稍后再试",
        }
    ...
    def run_play():
        recorder.play_recording(file_path, interval, input_text)
...
def play_inline(request: PlayInlineRequest):
    ...
    success = recorder.play_from_data(clicks_with_inputs, interval=request.interval)
    ...
    return {
        "status": "error",
        "message": "Failed to start inline playback",
    }
